...
export default function TasksPage() {
  ...
  const [editingSubType, setEditingSubType] = useState<number>(0);
  const [formSelectedSubType, setFormSelectedSubType] = useState<number>(0);
  ...
  const handleView = (task: Task) => {
    ...
    setEditedTask({ ...task });
    setEditingSubType(task.subType);
    setIsModalOpen(true);
    setIsExpanded(false);
    setIsEditing(false);
    setShowMenu(false);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mb-20 gap-4">
            {getFilteredTasks().map((task) => (
              <div
                ...
                className="bg-gray-800 p-4 rounded-lg shadow hover:shadow-md transition-all cursor-pointer h-[200px] flex flex-col"
              >
                ...
                {task.content && (
                  <p className="text-gray-400 mb-4 line-clamp-2">{task.content}</p>
                )}
                  ...
                  {task.startedAt && task.dueTime && (
                    <div className="flex items-center gap-2 text-sm text-gray-400">
                      ...
                      <span>
                        ...
                        {isOverdue(task.dueTime) && Number(task.status) !== Status.COMPLETED && (
              ...
              <div className="mt-4 pt-4 border-t border-gray-700 flex justify-between items-center sticky bottom-0 bg-gray-800">
                {isEditing ? (
                  ...
                  <span className="text-xs text-gray-400">
                    Created:{" "}
                    {format(new Date(selectedTask.startedAt), "dd/MM/yyyy HH:mm", {
                      locale: vi,
                    })}
                  </span>
                )}
