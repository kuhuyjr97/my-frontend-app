...
function TypeRow({ row, onEdited, onDeleted }: { row: TypeEnumRow; onEdited: () => void; onDeleted: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 group" style={{ borderBottom: '1px solid #f7f6f3' }}>
      ...
      <span className="text-[11px] tabular-nums shrink-0" style={{ color: '#ccc' }}>#{row.subType}</span>
  ...
  return (
    <div className="bg-white rounded-[14px] overflow-hidden" style={{ border: '1px solid #e8e6e1' }}>
      ...
      >
        ...
        <span className="text-[11px] tabular-nums" style={{ color: '#bbb' }}>{rows.length} loại</span>
      </div>
      {rows.length === 0 ? (
        <div className="px-4 py-3 text-[12px]" style={{ color: '#bbb' }}>Chưa có loại nào — bấm + để thêm</div>
      ) : (
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-5 flex flex-col gap-5 max-w-[800px] mx-auto w-full pb-[max(1.5rem,env(safe-area-inset-bottom))]" style={{ paddingTop: 72 }}>
        ...
        {/* ── Tài khoản ── */}
        <div className="bg-white rounded-[14px] overflow-hidden" style={{ border: '1px solid #e8e6e1' }}>
          <div className="px-4 py-3" style={{ borderBottom: '1px solid #f0eeea' }}>
            <span className="text-[13px] font-medium" style={{ color: '#1a1a1a' }}>Tài khoản</span>
          </div>
          <div className="flex items-center justify-between px-4 py-3">
            <div>
              <div className="text-[13px]" style={{ color: '#1a1a1a' }}>{username ?? '—'}</div>
              <div className="text-[11px] mt-0.5" style={{ color: '#bbb' }}>Đăng nhập hiện tại</div>
            </div>
            <button
              type="button"
              onClick={handleLogout}
              className="flex items-center gap-1.5 h-[30px] px-3 rounded-[7px] text-[12px] font-medium hover:bg-[#fbeaea] transition-colors"
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <span className="text-[13px] font-medium" style={{ color: '#1a1a1a' }}>Quản lý loại</span>
          </div>
          ...
          <div className="flex gap-1.5 flex-wrap mb-3">
            <button
              ...
              style={{ backgroundColor: filter === 0 ? '#1a1a1a' : '#f0eeea', color: filter === 0 ? '#fff' : '#555' }}
            >
              Tất cả
            </button>
            {CATEGORIES.map((c) => (
              <button
                ...
                className="h-[28px] px-3 rounded-[6px] text-[12px] font-medium transition-colors"
                style={{ backgroundColor: filter === c.type ? c.color : '#f0eeea', color: filter === c.type ? '#fff' : '#555' }}
              >
                {c.label}
              </button>
            ))}
